...
interface State {
  ...
  string: boolean;
  stringSingle: boolean;
  stringRaw: boolean;
  textBlockIndent?: number | null;
  stringDouble?: boolean;
}

export const jsonnet: StreamParser<State> = {
  name: 'jsonnet',
  languageData: {},
  startState: () => {
    return {
  ...
  token: (stream, state) => {
    ...
    if (state.string || state.importString) {
      const mode = state.string ? "string" : "meta";
      if (state.stringRaw) {
    ...
    const identifiers = stream.match(/[a-zA-Z_][a-zA-Z0-9_]*/) as RegExpMatchArray;
    if (identifiers) {
      const identifier = identifiers[0] as keyof typeof keywords;
      // If it's not in the dict, we return null to indicate no special
      // syntax highlighting.
      return keywords[identifier] !== undefined ? keywords[identifier] : null;
    }
